...
lbl = QtBind.createLabel(gui,'<font color="red">GAUCHE ©2025<font>',630,295)
lblMobs = QtBind.createLabel(gui,'#        SAYAÇTA GÖRMEZDEN       #\n#          GELİNECEK MOBLAR         #',31,6)
...
btnRemMob = QtBind.createButton(gui,'btnRemMob_clicked',"    SİL    ",119,258)
btnhakkinda = QtBind.createButton(gui,'btnhakkinda_clicked',"         HAKKINDA         ",590,265)
def btnhakkinda_clicked():
	log('\n\nHazarDungeon:\n * GAUCHE TARAFINDAN DUZENLENMISTIR. \n * FEEDBACK SISTEMLI BIR YAZILIMDIR. \n * HATA VE ONERI BILDIRIMLERINIZI BANA ULASTIRABILIRSINIZ.\n E-Mail:can.berk.cetin@hotmail.com.tr \n Instagram: can.berk.cetin \n\n\n    # BU PLUGIN ILE;\n - OYUN ICEROINDE BULUNAN TUM DUNGEONLARI SCRIPTE EKLENEN "AttackArea,X" KOMUTU ILE YAPABILIRSINIZ.\n - AYNI ZAMANDA OTO FGW TAS ACMA VE FGW CAGRI KABUL ETME SECENEKLERI MEVCUTTUR.\n - FGW TASI ACMAK ICIN SCRIPTE"GoDimensional" KOMUTU YAZDIRMANIZ YETERLIDIR.')
		...
		if HolyWaterTemple :
			if not str(QtBind.text(gui,lblSettings)) == "Holy Water Temple" :
				...
				if (OnlyCountUnique not in lstOnlyCount) or (OnlyCountUnique2 not in lstOnlyCount) :
					...
					saveConfigs()
					log("Plugin: HazarDungeon'ın Ayarları [ Holy Water Temple ] için ayarlandı.")
		elif ForgottenWorld :
			if not str(QtBind.text(gui,lblSettings)) == "Forgotten World":
				...
				if IgnoreMobs not in lstMobsData:
					...
					saveConfigs()
				log("Plugin: HazarDungeon'ın Ayarları [ Forgotten World ] için ayarlandı.")

txtFGWPROFILE = QtBind.createLineEdit(gui,'',155,291,70,20)
lbl = QtBind.createLabel(gui,'Profil İsmi : (Varsayılan FGW)',5,297)
btnSaveConfig = QtBind.createButton(gui,'onSaveButtonClicked',"KAYDET",5,275)
...
def onSaveButtonClicked():
    ...
    if timer is None or not timer.is_alive():
        ...
        timer.start()
        log('Plugin: Profil ismi: (%s) kaydedildi.'%(DefaultProfile))
    else:
        pass
...
QtBind.append(gui,lstMonsterCounter,'ISIM (CINS)') 

lblPreferences = QtBind.createLabel(gui,"#                      SAYAÇ TERCİHLERİ                     #",240,6)
lstIgnore = []
lstOnlyCount = []
...
cbxOnlyCountGeneral = QtBind.createCheckBox(gui,'cbxOnlyCountGeneral_clicked','YALNIZCA SAY',405,_y)
_y+=20
...
cbxOnlyCountChampion = QtBind.createCheckBox(gui,'cbxOnlyCountChampion_clicked','YALNIZCA SAY',405,_y)
_y+=20
...
cbxOnlyCountGiant = QtBind.createCheckBox(gui,'cbxOnlyCountGiant_clicked','YALNIZCA SAY',405,_y)
_y+=20
...
cbxOnlyCountTitan = QtBind.createCheckBox(gui,'cbxOnlyCountTitan_clicked','YALNIZCA SAY',405,_y)
_y+=20
...
cbxOnlyCountStrong = QtBind.createCheckBox(gui,'cbxOnlyCountStrong_clicked','YALNIZCA SAY',405,_y)
_y+=20
...
cbxOnlyCountElite = QtBind.createCheckBox(gui,'cbxOnlyCountElite_clicked','YALNIZCA SAY',405,_y)
_y+=20
...
cbxOnlyCountUnique = QtBind.createCheckBox(gui,'cbxOnlyCountUnique_clicked','YALNIZCA SAY',405,_y)
_y+=20
...
cbxOnlyCountParty = QtBind.createCheckBox(gui,'cbxOnlyCountParty_clicked','YALNIZCA SAY',405,_y)
_y+=20
...
cbxOnlyCountChampionParty = QtBind.createCheckBox(gui,'cbxOnlyCountChampionParty_clicked','YALNIZCA SAY',405,_y)
_y+=20
...
cbxOnlyCountGiantParty = QtBind.createCheckBox(gui,'cbxOnlyCountGiantParty_clicked','YALNIZCA SAY',405,_y)
_y+=20
lblUnique2 = QtBind.createLabel(gui,'Unique2 (24)',240,_y)
cbxIgnoreUnique2 = QtBind.createCheckBox(gui,'cbxIgnoreUnique2_clicked','YOKSAY',345,_y)
cbxOnlyCountUnique2 = QtBind.createCheckBox(gui,'cbxOnlyCountUnique2_clicked','YALNIZCA SAY',405,_y)


_y+=25
cbxAcceptForgottenWorld = QtBind.createCheckBox(gui,'cbxAcceptForgottenWorld_checked','FGW DAVETLERINI KABUL ET',240,_y)
_y+=20
cbxFGWHWT = QtBind.createCheckBox(gui,'cbxFGWHWT_checked','OTOMATIK YAPILANDIRMA ( FGW ) VE ( HWT )',240,_y)
_y+=20
cbx3star = QtBind.createCheckBox(gui,'cbx3star_checked','',240,298)
_y+=2
lbl = QtBind.createLabel(gui,'YAPILANDIRMA: ',460,_y)
lblSettings = QtBind.createLabel(gui,"VARSAYILAN",545,_y)
...
def AttackMobs(wait,isAttacking,position,radius):
	...
	if count > 0:
		...
		if not isAttacking:
			start_bot()
			log("Plugin: ATAK BASLADI : ("+str(count)+") MOB ALANDA VAR. RADIUS: "+(str(radius) if radius != None else "MAX."))
		
		Timer(wait,AttackMobs,[wait,True,position,radius]).start()
	else:
		log("Plugin: MOBLAR TEMIZLENDI..")
		...
		log("Plugin: SCRIPTE GERI DONULUYOR...")
...
def WaitPickableDrops(filterCursor,waiting=0):
	...
	if waiting >= WAIT_DROPS_DELAY_MAX:
		log("Plugin: TOPLAMA SURESI SONA ERDI.")
		return
	...
	if drops:
		...
		for key in drops:
			...
			if IsPickable(filterCursor,value['model']):
				...
				break
		if drop:
			log('Plugin: TOPLAMASI ICIN BEKLENIYOR: "'+drop['name']+'"...')
...
def EnterToDimensional(Name):
	...
	if uid != 0 :
		...
		if Enabled and SyncProfileFGW == 1 and SkipCommand == False :
			...
			log('Plugin: Tebrikler, 5 Saat sonra "Tekrar Giriş" Aktif edildi.')
		return
	elif uid == 0 :
		if Enabled and SyncProfileFGW == 1 and SkipCommand == False :
...
def GoDimensionalThread(Name):
	...
	if dimensionalItemActivated:
		...
		log('Plugin: '+( '"'+Name+'"' if Name else 'Dimensional Hole')+' HALA ACIK!')
	...
	if item:
		...
		if DimensionalActivated == False :
			...
			stop_bot()
			Timer(random.uniform(0.5,1),use_return_scroll).start()
			log('Plugin: '+( '"'+Name+'"' if Name else 'Dimensional Hole')+' ACMAK ICIN HALA ZAMAN VAR.')	
	else:
...
def DimensionalHole(ListDimensionalHole):
	if ListDimensionalHole == 'DimensionalHole':
		DimensionalHoleList = { "Dimension Hole (Shipwreck-★)": [], "Dimension Hole (Shipwreck-★★)": [], "Dimension Hole (Shipwreck-★★★)": [], "Dimension Hole (Shipwreck-★★★★)": [] }
...
if os.path.exists(getPath()):
	...
	try:
		...
		# Just in case omg -_-
		log('Plugin: CONFIG DOSYASI YUKLEME HATASI '+pName+' ')
else:
